  ...
  //public float camLerpSpeed = .05f;
	...
	void LerpPosition()
	{
		if (!hasAuthority)
		{
            ...
            float distance = Vector3.Distance(myTransform.position, syncPos);
            myTransform.position = Vector3.Lerp(myTransform.position, syncPos, (Time.deltaTime * lerpRate)/distance);
	...
	{
		if (hasAuthority && Quaternion.Angle(myTransform.rotation,syncRot) < rotThreshold || Vector3.Distance(myTransform.position, syncPos) < threshold)
		{
